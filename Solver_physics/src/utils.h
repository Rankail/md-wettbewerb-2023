...
struct Result {
	...
	Result(std::vector<std::shared_ptr<Circle>> circles, double A, double D, double B, int circleCountAtMax, double w, double h)
		: circles(circles), A(A), D(D), B(B), circleCountAtMax(circleCountAtMax), w(w), h(h) { }
